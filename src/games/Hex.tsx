...
const BOARD_SIZE = 11;
const HEX_RADIUS = 13.5;
const HEX_WIDTH = Math.sqrt(3) * HEX_RADIUS; // ~23.38
const ROW_STEP = HEX_RADIUS * 1.5; // ~20.25
...
export const Hex: React.FC = () => {
  ...
  const handleCellClick = (r: number, c: number) => {
    ...
    if (winPath) {
      ...
      // Toggle Turn without any disruptive modal
  ...
  return (
    <div className="flex flex-col h-full w-full justify-between overflow-hidden select-none">
      <GameHeader
        ...
        gameName="Hex"
        turn={turn}
        statusText={`Player ${turn}'s Turn (${turn === 1 ? 'Connect Blue Top-Bottom' : 'Connect Red Left-Right'})`}
        subStatusText={
          winner !== null
            ? `Player ${winner} formed an unbroken path and wins!`
            : `Move ${moveCount + 1}: Tap any vacant hex to place an acrylic gem`
        }
      />
      ...
      <main className="flex-1 flex flex-col items-center justify-center p-2 sm:p-4 overflow-hidden">
        ...
        <div className="w-full max-w-sm flex items-center justify-between px-3 py-1.5 bg-container-dark/95 border border-[#3e444c] rounded-2xl shadow-xl mb-1.5 text-xs">
          ...
          <div className="flex items-center gap-1.5">
            <span className="font-bold text-red-300">P2: Left ⇄ Right</span>
            <span className="w-3 h-3 rounded-md bg-red-500 shadow-sm inline-block" />
          </div>
        </div>

        {/* 11x11 Rhombic Board in Mahogany Wood Frame */}
        <div className="relative flex items-center justify-center max-h-[72vh] aspect-[470/360] clubhouse-board-depth table-flat rounded-3xl overflow-hidden shadow-2xl border-4 border-[#3e2312] bg-[#29170a]">
          <svg
          ...
          >
            ...
            <line
              x1={startX - 12}
              y1={startY - 12}
              x2={startX + (BOARD_SIZE - 1) * HEX_WIDTH + 14}
              y2={startY - 12}
              stroke="#2563eb"
              strokeWidth="7"
              strokeLinecap="round"
            ...
            <line
              x1={startX + ((BOARD_SIZE - 1) * 0.5) * HEX_WIDTH - 14}
              y1={startY + (BOARD_SIZE - 1) * ROW_STEP + 14}
              x2={startX + ((BOARD_SIZE - 1) * 1.5) * HEX_WIDTH + 14}
              y2={startY + (BOARD_SIZE - 1) * ROW_STEP + 14}
              stroke="#2563eb"
              strokeWidth="7"
              strokeLinecap="round"
            />
            ...
            <line
              x1={startX - 14}
              y1={startY - 10}
              x2={startX + ((BOARD_SIZE - 1) * 0.5) * HEX_WIDTH - 14}
              y2={startY + (BOARD_SIZE - 1) * ROW_STEP + 14}
              stroke="#dc2626"
              strokeWidth="7"
              strokeLinecap="round"
            ...
            <line
              x1={startX + (BOARD_SIZE - 1) * HEX_WIDTH + 16}
              y1={startY - 10}
              x2={startX + ((BOARD_SIZE - 1) * 1.5) * HEX_WIDTH + 16}
              y2={startY + (BOARD_SIZE - 1) * ROW_STEP + 14}
              stroke="#dc2626"
              strokeWidth="7"
              strokeLinecap="round"
